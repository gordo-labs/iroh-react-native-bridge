...
class NativeIrohMobileBridge : public jsi::HostObject {
  private:
  ...
  protected:
    ...
    jsi::Value cpp_uniffi_iroh_mobile_bridge_fn_func_start(jsi::Runtime& rt, const jsi::Value& thisVal, const jsi::Value* args, size_t count);
    jsi::Value cpp_uniffi_iroh_mobile_bridge_fn_func_stop(jsi::Runtime& rt, const jsi::Value& thisVal, const jsi::Value* args, size_t count);
    ...
    jsi::Value cpp_uniffi_iroh_mobile_bridge_checksum_func_start(jsi::Runtime& rt, const jsi::Value& thisVal, const jsi::Value* args, size_t count);
    jsi::Value cpp_uniffi_iroh_mobile_bridge_checksum_func_stop(jsi::Runtime& rt, const jsi::Value& thisVal, const jsi::Value* args, size_t count);
    jsi::Value cpp_ffi_iroh_mobile_bridge_uniffi_contract_version(jsi::Runtime& rt, const jsi::Value& thisVal, const jsi::Value* args, size_t count);

  public:
    NativeIrohMobileBridge(jsi::Runtime &rt, std::shared_ptr<uniffi_runtime::UniffiCallInvoker> callInvoker);
    virtual ~NativeIrohMobileBridge();

    /**
     * The entry point into the crate.
     *
     * React Native must call `NativeIrohMobileBridge.registerModule(rt, callInvoker)` before using
     ...
     *
     * Clients should call `NativeIrohMobileBridge.unregisterModule(rt)` after final use where possible.
     */
